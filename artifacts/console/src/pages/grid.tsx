...
import { useListVms, useGetLines, useUpdateLine, useResetLines } from "@workspace/api-client-react";
import { useSSE } from "@/hooks/use-sse";
import { VmTile } from "@/components/vm-tile";
import { Button } from "@/components/ui/button";
import { RotateCcw, AlertTriangle, MonitorPlay, Maximize2, LayoutGrid } from "lucide-react";
...
export function GridPage() {
  ...
  const getLineState = (vmId: number) => {
    ...
    return lines[stringId] || initialLines?.find(l => l.id === stringId) || { id: stringId, state: 'idle', label: '', updatedAt: new Date().toISOString() };
  };
  ...
  if (loadingVms) {
    return <div className="flex-1 flex items-center justify-center"><div className="animate-pulse text-muted-foreground">LOADING COGITATORS...</div></div>;
  }
    ...
    <div className="flex flex-col h-full bg-[#0a0a0a]">
      ...
      <header className="h-14 px-4 border-b border-border bg-card flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" onClick={handleResetAll} className="h-8 gap-2 text-destructive border-destructive hover:bg-destructive hover:text-destructive-foreground">
          ...
          <div className="flex items-center gap-1 border border-border rounded-md p-0.5 bg-background">
            {[2, 3, 4, 5].map(c => (
              <button
                ...
                className={`w-8 h-7 text-xs rounded-sm font-medium transition-colors ${columns === c ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                {c}
              </button>
            ))}
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${status === 'SYNCED' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : status === 'OFFLINE' ? 'bg-yellow-500 animate-pulse' : 'bg-gray-500'}`} />
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {vms?.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="max-w-md text-center mb-6">Go to Configuration to add Dante VMs to the console.</p>
          </div>
        ) : (
          <div 
